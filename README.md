...
# 1. ツール概要
主に映像作品の演出を整理・蓄積し、素早く参照できるようにする C# + Excel VBA のツールです。
<br>
<br>
<br>
...
# 2.  ツールを作った背景
私は趣味で楽曲のMVを制作しています。  
制作時、依頼者と「どのような演出がよいか」を相談しますが、たとえば依頼者から「ここでキャラクターが走ってほしい」と言われても、「走る」にも多様な表現があり、最適な方向性の判断に迷うことがあります。毎回、会議中に多数のMVを見返して「走る」演出を探すのは時間がかかります。  
そこで、あらかじめ多様な「走る演出」をまとめたリファレンスを共有できれば、**共通認識を素早く形成**でき、認識の齟齬を防げると考えました。  
この発想を拡張し、**さまざまな映像作品を“演出ごと”に整理できるツール**として本ツールを作成しました。  
<br>
<br>
<br>
...
## 単語別に演出をまとめる
- 「単語生成」ボタンを押すと、一覧にまとめられている各分析メモシートに記録された演出を単語別にシートに書き出します。
- 同時に単語数等を整理した「単語まとめ」シートも作成されます。
...
| 記録ボタンを押すと、設定範囲のスクリーンショットと撮り、画面左のフォームに入れた単語と共に、設定したエクセルに書き込みます。 | 記録するExcelファイルや秒数指定のリンク、スクショの縦横サイズ等を設定できます。 |
...
| 各分析メモシートをまとめたシートです。右上のマクロとリンクしたボタンから、分析メモシートを作成したり、演出を単語別にまとめることができます。 | タイトルごとに演出を記録します。SsRecorder：記録画面を使って記録していく場所はここになります。 |
...
| 分析まとめ：一覧の「単語生成」ボタンから自動生成されます。各単語の登場数とシートへのリンクをまとめています。 | 分析まとめ：一覧の「単語生成」ボタンから自動生成されます。分析メモ：メモシートに記録した演出を単語別にシートにまとめています。 |
<br>
<br>
<br>

# 5.  動作環境
- Excel 2024 もしくは Office 365
- `.exe` が実行できる Windows 環境
<br>
<br>
<br>
...
│  ├─ Model/                       # データ関連の処理
│  ├─ Module/                      # 単機能モジュール（例：画像解像度変更）
│  ├─ SsRecorder/                  # アプリ固有（他で流用しない想定）
│  ├─ View/                        # UI・フォーム
│  └─ Program.cs                   # エントリーポイント
├─ SsRecorder_Excel/               # VBAアプリ
├─ SsRecorder_Excel_Sample/        # ツールで作成したサンプルExcel
...
- 当初は個人利用を前提として、Google スプレッドシート（GAS）で作成しておりました。
    - 作成したもの：後でURLを記載予定です。
- しかし、スプレッドシート運用には以下の課題がございました。
    - 配布時に**受け手側で Google Cloud の設定が必要**となり、非エンジニアの方にとってはハードルが高いこと。
    - *GAS の実行上限（5分）**があるため、データが増えると処理が途中で止まってしまう懸念があること。
- これらの課題を解消するため、**Excel ベースで全面的に作り直しました。**
<br>
## SsRecorder の中核処理を View（フォーム）に書かない
- 将来的に UI を改善することを前提とし、**フォームにロジックを集中させないようにいたしました。**
- これにより、改修時の影響範囲を最小化できるよう、**責務の分離を意識した設計**にしております。
<br>
## エンジニア以外でも使いやすいツールにした点
- 本ツールの想定ユーザーは**デザイナーやアニメーター**の方々です。
- そのため、DB・Google Cloud・AWS などを前提としたツールにしてしまうと、ユーザーにとって利用のハードルが高くなります。
- そこで、**多くの方に馴染みのある Excel でデータを管理**し、**直接編集も可能な設計**といたしました。
<br>
<br>
<br>
...
# 10. 今後の動きについて
- 入力や I/O 処理など、**バリデーションを強化**してまいります。
- **Mac でも利用可能な形**を検討し、以下の取り組みを行う予定です。
    - **Avalonia UI** による再実装
    - 設計は **MVVM** を採用
- **UI の改善**を進め、より使いやすい操作性を目指します。
- **Google Cloud Speech-to-Text** を用いた音声入力機能の追加も検討しております。
<br>
<br>
<br>
